...
- `needs_display_recheck`：表示確認から10分超過、確認情報不足、または反映・起動記録が更新。10分以内で成果物・表示コード・反映・起動記録が変わらない実確認は再利用する。
...
DB先行保存構成でのRSS通信失敗は、明示された実行IDに限り `preflight --kind collect --retry-failed-execution ID`、続いて `collect --retry-failed-execution ID` で復旧できる。保存前失敗・既存出力なし・DB保存なしを再検査し、n8n公式retryOfを保持する。結果不明の再送、既存ガード削除、成功済み収集の再実行は許可しない。詳細は [フェーズ7保存手順](database-phase7-writes.md#rss取得のタイムアウト対策と限定再開)。
...
本文取得の既定値はGoogle 30秒/リクエスト、記事サイト10秒/ホスト、全体2秒。同時実行は従来どおり1プロセス。Google URLの解決は複数リクエストになるため記事単位の間隔ではない。リダイレクトも対象とし、通常の次回アクセス時刻と429の待機をディスクへ保存する。429では最低60秒・段階的なバックオフ・Retry-Afterのうち最も長い時間を守る。一般エラーの最大試行は2回。取得先の共有IP制限等もあるため429ゼロの保証値ではない。遅い代わりにアクセス集中を抑える。

取得・予定した再取得→変更記事のみ共通レビュー更新→要約・人材・分類確定→週次→DB反映→最終画面確認の順とする。待機中に既存のcurrent/readyレビューを再利用する。取得コマンドの `--write` は未作成日だけ自動下書きを保存し、既存要約は保護する。

キーワードは通常メタデータ入力だけに依存する。本文・共通レビューを根拠に使う場合は追加evidenceへ指定する。以前の広い依存関係で保存したcheckpointは自動承認し直さず、必要時に一度再確認する。

起動確認の待機時間内に正常応答しなくてもプロセスが稼働中なら、startは `starting` と30秒後のresume指示を返す。成功扱いにも再起動にもせず、既存プロセスの状態を確認する。

Windows/WSLの既知sandbox初期化エラーへの標準実行方法は `.agents/skills/autoarticle-operations/SKILL.md` の「このWindows・WSL環境の実行方法」を参照する。
