...
## このWindows・WSL環境の実行方法

- この環境では通常sandboxがコマンド実行前に `helper_unknown_error: setup refresh had errors` で失敗する。ユーザー指定により、シェル実行は成功実績のある `exec_command` の `sandbox_permissions: require_escalated` と具体的なjustificationを最初から使い、PowerShellから `wsl -d Ubuntu --cd プロジェクトルート ...` を実行する。通常sandboxへの失敗確認を毎回繰り返さない。これは承認経路の指定であり、審査拒否や権限を迂回しない。別環境へ一般化しない。
- Chrome表示確認は既存の直接起動スクリプトを使う。署名制限がある場合は実績のある `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force` を当該PowerShellプロセスにだけ適用する。画像の読取ツールが同じsandbox初期化エラーで失敗する環境では、承認されたシェルでPNGを読み出して画像表示する。

## 運用手順

1. **n8nの準備**：起動が依頼範囲なら `python3 scripts/autoarticle_ops.py start n8n`。既存起動を再利用し、必要時だけログ付きで常駐起動する。`starting` が返った場合は30秒後に `resume` で既存プロセスの起動を確認し、二重起動しない。起動待ちだけで障害調査へ進まない。継続して起動しない場合のみ対象ログを調べる。続けて `status` のactive・schedule・timezone・definitionMatchesを確認する。設定変更による再起動・同期は別途影響を確認する。「備える」だけなら収集せずここで終える。
...
4. **日次記事作業**：生成指示書の対象日 `sourceRunDate`（旧形式は収集記録と照合）を使う。記事要約、人材索引、分類、キーワード抽出の指示書を順次読み、`rulesPath` とその必須参照を読む。入力は `scripts/read_ai_inputs.py` の対応taskで分割取得し、記事・本文の継続を必要範囲まで確認する。本文未取得なら取得処理へ進み、コマンド確認は `python3 scripts/capture_article_contents.py --help`。ローカル取得だけなら `--no-sync-contents`、DB同期は反映範囲に含まれる場合だけ行う。
   取得中は候補の整理と取得済み記事の共通レビューを進める。取得不能の記事を自動再取得せず、初回取得と保存状態を確認してから、要約・人材・分類を確定しcheckpointする。保存済み工程は本文・根拠・ルール変更でも再確認しない。未完了だけをinputVersion 2で読み、readyの工程別根拠を利用する。`--write` は未作成日の自動下書きだけを作り、既存要約を上書きしない。保存済み要約への追記・修正はレビュー工程で行う。
   要約工程で `docs/ai-rules/shared-article-review.md` に沿う共通記録を `scripts/save_article_review_facts.py` で検証・保存し、人材・分類で再利用する。未完了のstate: readyは工程別根拠を再利用し、heldは関連するmissingTopicsの根拠追加時だけ再開する。needs_reviewで初回本文確認を行う。自動下書きや取得成功を意味的なレビュー完了とせず、未確認の工程をreadyにしない。各指示書の出力契約どおり保存・検証する。
...
7. **確認アプリ**：成果物確定と依頼されたDB反映が終わってから、最終画面をまとめて確認する。`python3 scripts/autoarticle_ops.py start dashboard`。Windowsでは `scripts/verify_dashboard_chrome.ps1` をPowerShellで直接実行し、Chromeの独立プロファイルで返されたURL（既定 `http://127.0.0.1:8765/`）を描画する。DOMとスクリーンショットで、対象日の記事・要約・分類・週次成果物と表示データソースを確認する。DB表示か提案JSONの代替表示か不明なら `docs/talent-dashboard.md` を読む。HTTP疎通だけを表示確認完了とせず、Chromeの直接起動を第一手段とし、初期化エラーが既知の操作ツールを先に試さない。成果物を取得できても内容を確認するまでpageを完了にしない。直接起動できなければURLと未確認範囲を返す。
...
- 判断に必要な記事は未完了一覧のoffsetで詳細を読み、短いrefを保存する。`--view detail --offset N --limit 1 --max-content-chars 4000` で読む。必要な本文の続きは `nextOffset`（本文内）に従う。タイトルが切られた記事も詳細で確認する。既読の共通記録を再利用し、未確認記事を飛ばさない。
...
キーワードがタイトル・RSS抜粋だけを根拠とする場合、本文取得・共通レビューの更新では失効しない。本文や共通レビューも判断に使った場合は、そのファイルをcheckpointの追加 `--evidence` に指定して変更検出を維持する。
...
- 再開時は `resume` で現在の入力・成果物・DB・実行履歴と照合する。`stale` / `unverified` は再確認、`submission_unknown` は実行有無の調査、`needs_display_recheck` は画面再確認。実確認から10分以内で成果物・画面コード・反映・起動記録が不変なら、直後のresumeは確認済みを再利用する。記録なしは未確認。再開コマンド自体は操作を再実行しない。保留・暫定を含む工程完了と全記事レビュー完了を混同しない。

## 記事作業と反映を途中で保留しないための進め方

- 全工程の依頼に「記事作業」「レビュー済み提案のDB反映」が含まれていれば、その範囲は承認済み。提案の保存・レビュー・検証後に反映まで進め、同じ許可を再確認する理由で止めない。
- 本文未取得・部分取得は記事単位の保留として記録する。本文取得は同時1件、Googleは30秒、記事サイトはホストごと10秒、全体2秒間隔を既定とする。リダイレクト先にも間隔を適用し、速度を上げるために待機状態を消さない。HTTP 429のホストは最低1分休止し、Retry-Afterを守って再試行する。取得不能（unavailable / metadata_only）と記録した記事は既存のretry_afterを含め自動再実行しない。失敗履歴と理由を保持して他工程へ進む。待機する場合は60秒以下の区切りで状況を確認する。同じ実行中に再試行上限へ繰り返し達する場合は、取得不能の理由を残し、自動再取得対象から外す。取得可能な他ホストの処理、全記事の登録候補化、根拠のある人物関係・分類、キーワード・週次作業を継続する。未確認記事をreadyへ変えることで解消しない。
...
inputVersion 2の未完了一覧だけを処理し、excludedのsaved/held/unavailableをAI再判断へ戻さない。短いrefは対象日・工程・記事・本文版・確認記録に固定される。追加参照は --article-ref REF --reference-kind body|facts|evidence|source で不足部分だけをDBから読む。保存中に対象集合が変わったらoffset 0から取り直す。

事実のtaskFactsと保留のholds.missingTopics、refでの成果物保存は共通ルールに従う。人材の該当なしもreviewedArticlesで対象記事を明示する。保存済み未反映は既存成果物で後続処理を再開し、現在の本文やルールで過去根拠を書き換えない。
