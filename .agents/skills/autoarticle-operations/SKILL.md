...
7. **確認アプリ**：`python3 scripts/autoarticle_ops.py start dashboard`。Windowsでは `scripts/verify_dashboard_chrome.ps1` をPowerShellで直接実行し、Chromeの独立プロファイルで返されたURL（既定 `http://127.0.0.1:8765/`）を描画する。DOMとスクリーンショットで、対象日の記事・要約・分類・週次成果物と表示データソースを確認する。DB表示か提案JSONの代替表示か不明なら `docs/talent-dashboard.md` を読む。HTTP疎通だけを表示確認完了とせず、Chromeの直接起動を第一手段とし、初期化エラーが既知の操作ツールを先に試さない。成果物を取得できても内容を確認するまでpageを完了にしない。直接起動できなければURLと未確認範囲を返す。
...
- 本文未取得・部分取得は記事単位の保留として記録する。HTTP 429は取得間隔を広げ、Retry-Afterを守って再試行する。長い待機・試行上限では再開予定が保存されるため、nextRetryAtまで他工程を進め、期限後に同じ取得コマンドを再実行する。待機する場合は60秒以下の区切りで状況を確認する。同じ実行中に再試行上限へ繰り返し達する場合は、次回へ予定を引き継ぎ未取得を明記する。取得可能な他ホストの処理、全記事の登録候補化、根拠のある人物関係・分類、キーワード・週次作業を継続する。未確認記事をreadyへ変えることで解消しない。
