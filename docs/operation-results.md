...
`read_ai_inputs.py --run-date YYYY-MM-DD --task article-summary --view inventory --limit 50` は記事の移動用一覧。URL、本文、引用、レビュー入力ハッシュ、全検索語の再掲を省き、状態・タイトル・recordOffsetを返す。長いタイトル・理由の省略は明示する。全体のnextOffsetでページを進める。

意味的レビューは `--view detail --offset N --limit 1 --max-content-chars 4000` で根拠を読んで行う。既存の詳細出力は既定のまま保持。URLで選択した場合は同じ選択条件も維持する。inventoryから引用やready判定を作らない。

本文取得は既定で50件ごとと終了時に件数を表示し、個別タイトルは`--verbose`指定時だけ表示する。状態の詳細は保存済みキャプチャへ残り、同期等のエラーは標準エラーへ出す。件数のverifiedは取得状態であり編集レビュー完了ではない。
