...
本書は **フェーズ7完了時の保存契約と切戻し手順**。通常収集184で525件のDB先行保存と互換反映を確認した。現在は [フェーズ8](database-phase8-retirement.md) が完了し、旧側への自動互換反映を停止している。以下の `compatibility: complete` はautomatic方針時の契約であり、現行のon-demand方針・未出力数の扱いはフェーズ8の手順を優先する。
