...
4. **日次記事作業**：生成指示書の対象日 `sourceRunDate`（旧形式は収集記録と照合）を使う。記事要約、人材索引、分類、キーワード抽出の指示書を順次読み、`rulesPath` とその必須参照を読む。入力は `scripts/read_ai_inputs.py` の対応taskで分割取得し、記事・本文の継続を必要範囲まで確認する。本文未取得なら取得処理へ進み、コマンド確認は `python3 scripts/capture_article_contents.py --help`。ローカル取得だけなら `--no-sync-contents`、DB同期は反映範囲に含まれる場合だけ行う。
   取得中は候補の整理と取得済み記事の共通レビューを進める。予定した再取得が終了するか次回へ繰り越すことを決めてから、要約・人材・分類を確定しcheckpointする。再取得後は共通記録の入力ハッシュが変わった記事だけ再確認し、current/readyを再利用する。`--write` は未作成日の自動下書きだけを作り、既存要約を上書きしない。保存済み要約への追記・修正はレビュー工程で行う。
...
- 本文未取得・部分取得は記事単位の保留として記録する。本文取得は同時1件、Googleは30秒、記事サイトはホストごと10秒、全体2秒間隔を既定とする。リダイレクト先にも間隔を適用し、速度を上げるために待機状態を消さない。HTTP 429のホストは最低1分休止し、Retry-Afterを守って再試行する。長い待機・試行上限では再開予定が保存されるため、nextRetryAtまで他工程を進め、期限後に同じ取得コマンドを再実行する。待機する場合は60秒以下の区切りで状況を確認する。同じ実行中に再試行上限へ繰り返し達する場合は、次回へ予定を引き継ぎ未取得を明記する。取得可能な他ホストの処理、全記事の登録候補化、根拠のある人物関係・分類、キーワード・週次作業を継続する。未確認記事をreadyへ変えることで解消しない。
